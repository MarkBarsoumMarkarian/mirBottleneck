test_that(".build_mirna_expr returns named list of patient vectors", {
  ...
  # hsa-mir-21 should average rows 1 and 2 per patient
  result <- mirBottleneck:::.build_mirna_expr(mirna_log, mirna_norm_map)

  expect_type(result, "list")
  expect_named(result, c("hsa-mir-21", "hsa-mir-155"), ignore.order = TRUE)

  # hsa-mir-21: mean of (1,4), (2,5), (3,6) → c(2.5, 3.5, 4.5)
  expect_equal(result[["hsa-mir-21"]], c(P1 = 2.5, P2 = 3.5, P3 = 4.5))

  # hsa-mir-155: single row, no averaging needed
...
test_that("normalize_01 handles zero-variance input", {
  x      <- c(5, 5, 5)
  result <- mirBottleneck:::normalize_01(x)
